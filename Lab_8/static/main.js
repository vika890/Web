...
function perPageBtnHandler(event) { // выбор кол-во элементов по странице
    let searchQuery = document.querySelector('#search-input').value.trim();
...
function searchRecords(query) { // получение записей, соответствующих поиску
    fetch(`http://cat-facts-api.std-900.ist.mospolytech.ru/facts?q=${query}`)
        .then(response => response.json())
        .then(data => {
            console.log('Вы искали:', data);
...
function showAutocompleteSuggestions(suggestions) { // отображает список подсказок
